...
string to4hex(unsigned long long n)
{
  char buffer[80];

   snprintf(buffer, sizeof(buffer), "%04llX", n);  

  return string(buffer);
}

string to2hex(unsigned  long n)
{
  char buffer[80];

   snprintf(buffer, sizeof(buffer), "%02llX", n);  
